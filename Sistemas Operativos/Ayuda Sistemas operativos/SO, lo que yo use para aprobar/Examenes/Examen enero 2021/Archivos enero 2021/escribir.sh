...
#----------------------------------
	...
	if [ $# -ne 2 ]; then
		echo "Error en el numero de parametros deben de ser 2."
		sintaxis
		exit 1
	fi
	...
	if [ $1 -lt $MIN_HIJOS ] || [ $2 -gt $MAX_HIJOS ]; then
		echo "El valo debe de estar entre $MIN_HIJOS y $MAX_HIJOS"
	  	Sintaxis
	  	exit 1
	fi
	#Comprobamos que el parámetro numJobs es un número
	if ! Number $12; then
		echo "El parámetro <numJobs> ($2) debe ser un número"
	  	Sintaxis
	  	exit 1
	fi
	#Comprobamos el rango del parámetro numJobs
	if [ $1 -lt $MIN_JOBS ] || [ $2 -gt $MAX_JOBS ]; then
		echo "El valo debe de estar entre $MIN_JOBS y $MAX_JOBS"
	  	Sintaxis
	  	exit 1
	fi
#----------------------------------
