...
LoadResults(){
	...
	#Se incrementa en uno el valor del array letras en el índice calculado
		...
		while read -n1 c; 
		do
			orden=`ord $c` #ord es para sacar el valor numerico en asco, es decir, chr 65 = A y ord A = 65
			let index=$orden-65
			#Para guardar el valor de una operación dentro de una variable usamos el comando let. 
			#El comando evalúa cada argumento como una expresión aritmética.
